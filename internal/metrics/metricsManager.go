...
const (
	...
	counterDescriptionOperationDurationMiliSeconds = "Histogram/sum/count of operation time for specific operation, by name and requestId"
...
type MetricsManager struct {
	...
	serviceLogicOperationDurationMiliSeconds *prometheus.GaugeVec
...
func NewMetricsManager(registry prometheus.Registerer) *MetricsManager {
	...
	m := &MetricsManager{
		...
		serviceLogicOperationDurationMiliSeconds: prometheus.NewGaugeVec(prometheus.GaugeOpts{
			Namespace: namespace,
			Subsystem: subsystem,
...
func (m *MetricsManager) Duration(operation string, duration time.Duration) {
	m.serviceLogicOperationDurationMiliSeconds.WithLabelValues(operation).Add(float64(duration.Milliseconds()))
}
