...
func getAgentMac(agent *v1alpha1.Agent) string {
...
var _ = Describe("[kube-api]cluster installation", func() {
	...
	It("deploy clusterDeployment with agent and invalid installer args", func() {
		...
		Eventually(func() bool {
			condition := conditionsv1.FindStatusCondition(getAgentCRD(ctx, kubeClient, key).Status.Conditions, v1alpha1.AgentSyncedCondition)
			if condition != nil {
	...
	It("deploy clusterDeployment with agent,bmh and installer args", func() {
		...
		h, err := common.GetHostFromDB(db,
			cluster.ID.String(), host.ID.String())
		Expect(err).To(BeNil())
		Expect(h.InstallerArgs).To(BeEmpty())
